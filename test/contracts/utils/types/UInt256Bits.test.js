...
const UInt256Bits = contract.fromArtifact('UInt256BitsMock');
...
describe('UInt256Bits', function () {
    ...
    for (const integer of integers) {
        for (const value of values) {
            it(
                `matches after inject/extract: integer=${integer}, value=${value.value}, length=${value.length}, index=${value.index}`,
                async function () {
